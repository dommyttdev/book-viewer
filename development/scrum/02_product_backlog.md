...
| PBI-108 | 退会を実装する | 利用終了時に一般ユーザのアカウント状態と個人機能データを安全に整理できる。 | 本人確認、withdrawn状態、セッション失効、お気に入りと閲覧履歴の無効化 | 中 |
| PBI-109 | 本削除を実装する | 不要な本を一覧、検索、閲覧対象から安全に除外できる。 | 論理削除、権限制御、検索除外、派生ファイル整理余地、整合性確認 | 中 |
| PBI-110 | 変換運用確認を強化する | 管理ユーザが変換失敗やページ順を確認しやすくなる。 | 失敗理由、状態絞り込み、ページ順確認、権限制御、ログの安全性 | 中 |
| PBI-111 | 著者、タグ、シリーズ、種別のマスタ管理を実装する | メタ情報の表記揺れや分類を管理しやすくする。 | 重複防止、統合余地、参照中データの保護、認可 | 低 |
| PBI-112 | 一般ユーザ管理を強化する | 管理ユーザが一般ユーザの状態を確認し、必要に応じて停止できる。 | 一覧、停止、復帰、既存セッション失効、権限制御、監査観点 | 中 |
