...
| #87 | 最小テストと確認コマンドを整備する。 | #82から#86の実装に合わせて実施する。 | フロントエンド、API、Worker、ミドルウェアの最小確認コマンドが記録されている。 |
| #88 | ローカル開発手順とTODOを実装結果に合わせて更新する。 | 最後に実施する。 | `doc/05_development/03_environment_setup.md`、`doc/05_development/04_local_development.md`、`doc/TODO.md`、issue #48のチェックリストが実構成と整合している。 |
