...
システム外部の利用者や外部システムとの関係は `doc/03_architecture/04_system_context/01_system_context.md` で扱う。各ユースケースの詳細な処理順序は `doc/03_architecture/06_data_flow/01_data_flow.md` で扱う。
...
| Spring Boot変換ワーカー | Spring Boot 4.0.6 / Java 25 | 専用ジョブキューからジョブを取得し、アーカイブ展開、WebP変換、サムネイル生成、ジョブ状態更新を担当する。 |
...
| 専用ジョブキュー | 製品未定 | バックエンドAPIと変換ワーカーを非同期に接続し、変換ジョブを配送する。 |
...
| Spring BootバックエンドAPI | 専用ジョブキュー | キュー操作 | 変換ジョブを投入する。 |
| Spring BootバックエンドAPI | メール送信基盤 | HTTP / SMTPなど | メール認証、2段階認証、パスワードリセット通知を送信する。 |
| 変換ワーカー | 専用ジョブキュー | キュー操作 | 変換ジョブを取得し、完了、失敗、再試行を扱う。 |
...
専用ジョブキューは、バックエンドAPIと変換ワーカーを非同期に接続する。

主な責務は次のとおり。

- 変換ジョブを受け付ける
- 変換ワーカーへジョブを配送する
- 変換ワーカーの処理完了、失敗、再試行を扱う
- ジョブ滞留や失敗を運用上観測できるようにする

具体的なキュー製品、デッドレター、再試行回数、可観測性の詳細は、非同期変換ジョブ方式ADRと画像変換設計で決定する。
...
| 専用ジョブキューの永続データ | 製品選定後、ジョブ消失を避ける必要がある場合に保持する。 |
...
- 専用ジョブキューの具体的な製品選定
- API、Worker、ミドルウェアのDocker Composeサービス名とネットワーク設定
