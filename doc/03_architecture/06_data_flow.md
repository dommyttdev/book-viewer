...
コンテナ単位の責務と通信関係は [doc/03_architecture/05_container_diagram.md](05_container_diagram.md) で扱う。データモデル、API契約、ファイル保存、画像変換、検索、権限の詳細は、後続の設計ドキュメントで具体化する。
...
| 検索更新Outbox | 正本 | PostgreSQL | バックエンドAPI / 変換ワーカー / 再試行ワーカー / 管理コマンド | Elasticsearch更新失敗、再試行、再インデックスの判断に利用する。RabbitMQは通知、配送手段として扱う。 |
...
5. アップロード受付後、バックエンドAPIはRabbitMQへ変換ジョブを投入し、HTTPレスポンスでは変換完了を待たない。
...
7. RabbitMQのack、retry、dead letterの具体方式は、非同期変換ジョブ方式ADRと画像変換設計で管理する。
...
- PostgreSQLは書籍メタ情報、ページ情報、変換状態、検索更新Outboxの正本である。
...
flowchart LR
  ...
  worker -->|"Outbox記録"| outbox
  worker -->|"即時インデックス更新"| es
  worker -. 更新失敗 .-> outbox
  worker -. 再試行通知 .-> retry

  retry -. Outbox ID配送 .-> retryWorker
  retryWorker -->|"最新正本読み直し"| db
  retryWorker -->|"再試行結果更新"| outbox
  retryWorker -->|"再インデックス"| es
  command -->|"書籍単位 / 全件再インデックス"| db
  command -->|"再構築"| es
  command -->|"Outbox整理"| outbox
```

処理方針は次のとおり。

1. 書籍メタ情報を登録または更新する場合、バックエンドAPIはまずPostgreSQLを更新する。
2. 検索対象項目が変わった場合、バックエンドAPIはPostgreSQLに検索更新Outboxを記録し、コミット後にElasticsearchの該当ドキュメントを即時更新する。
3. 変換完了によりサムネイルや閲覧可能状態が変わる場合、変換ワーカーはPostgreSQLに検索更新Outboxを記録し、必要に応じてElasticsearchの該当ドキュメントを即時更新する。
4. Elasticsearch更新に成功した場合、Outboxを完了状態にする。
5. Elasticsearch更新に失敗した場合、Outboxを再試行待ちとして記録し、RabbitMQへOutbox IDを含む再試行通知を送る。
6. 再試行ワーカーはRabbitMQから受け取ったOutbox IDをもとにPostgreSQLを読み直し、最新正本から検索ドキュメントを再生成する。
7. 書籍単位の再インデックスと全件再インデックスを管理コマンドとして用意する。
8. Elasticsearchの内容が疑わしい場合は、PostgreSQLを基準にインデックスを破棄、再構築できるようにする。

RabbitMQ上のメッセージは検索更新状態の正本ではない。メッセージが重複配送された場合や失われた場合も、PostgreSQL上の検索更新Outboxを基準に再試行、再通知、手動再インデックスを判断する。
...
| Elasticsearch更新 | 検索更新Outboxを再試行待ちにし、再試行または再インデックスで回復する。 |
...
4. 更新失敗時はPostgreSQLの検索更新Outboxを再試行待ちにし、RabbitMQへ再試行通知を送る。
5. 再試行結果はPostgreSQLの検索更新Outboxに反映する。
...
- RabbitMQのexchange、queue、routing key、ack、retry、dead letter、可観測性
