...
| PostgreSQL | PostgreSQL | メタ情報、ユーザ、権限、ジョブ状態、検索更新Outbox、閲覧履歴、お気に入りなどの正本を保持する。 |
| Elasticsearch | Elasticsearch | タイトル、著者、タグ、シリーズなどの日本語検索用インデックスを保持する。必須プラグインは技術スタックを正本とする。 |
| RabbitMQ | RabbitMQ | 変換ジョブと検索更新Outbox IDを配送する。検索更新状態の正本はPostgreSQLに置く。 |
...
| Spring BootバックエンドAPI | Elasticsearch | HTTP | 検索要求、検索インデックスの即時更新要求を行う。 |
| Spring BootバックエンドAPI | 書籍ファイル保存領域 | ファイルI/O | 原本ファイル保存、変換済み画像やサムネイルの参照を行う。 |
| Spring BootバックエンドAPI | RabbitMQ | AMQP | 変換ジョブ、検索更新再試行通知を投入する。 |
...
| 変換ワーカー | Elasticsearch | HTTP | 変換完了後のインデックス即時更新を行う。 |
...
- 検索対象項目の変更時に検索更新Outboxを記録し、Elasticsearch更新を即時試行する
- 変換ジョブをRabbitMQへ投入する
...
- 変換完了後に検索更新Outboxを記録し、必要に応じてElasticsearchインデックスを即時更新する
...
RabbitMQは、バックエンドAPI、変換ワーカー、検索更新の再試行ワーカーを非同期に接続する配送手段として利用する。

主な責務は次のとおり。

- 変換ジョブを受け付ける
- 変換ワーカーへジョブを配送する
- 検索更新Outbox IDを再試行ワーカーへ配送する
- 変換ワーカーのack、再配送、dead letterを扱う
- ジョブ滞留や失敗を運用上観測できるようにする

業務上のジョブ状態はPostgreSQLの`conversion_job`を正本とし、検索更新状態はPostgreSQLの`search_index_outbox`を正本とする。RabbitMQには配送に必要な最小限のメッセージを置く。配送保証はat-least-onceとし、重複配送はPostgreSQLの状態確認により冪等に扱う。
...
| RabbitMQの永続データ | 変換ジョブメッセージと検索更新再試行通知の消失を避けるために保持する。ただし業務状態の正本ではない。 |
