...
| RabbitMQ | APIと変換ワーカーを非同期に接続し、変換ジョブを配送する。 |
...
- API、Worker、PostgreSQL、Elasticsearch、RabbitMQを将来的に分離する余地を残せる
...
4. バックエンドAPIがRabbitMQへ変換ジョブを投入する。
5. 変換ワーカーがRabbitMQからジョブを取得する。
