...
アプリケーションのディレクトリ構成、ビルドツール、パッケージマネージャ、主要な起動単位はSprint S0 / issue #81の設計判断に基づく。実装結果に差分が出た場合は、この文書を正として更新する。
...
- リポジトリ構成、アプリケーション配置、ビルド単位、起動単位は [プロジェクト構成](00_project_structure.md) を正本とする。
...
PostgreSQL、Elasticsearch、RabbitMQなどのミドルウェアはDocker Composeで起動する。
...
バックエンドAPIは、認証、書籍管理、検索、閲覧、管理操作、PostgreSQL更新、Elasticsearch検索、RabbitMQへのジョブ投入を担当する。
...
変換ワーカーは、RabbitMQからジョブを取得し、アーカイブ展開、WebP変換、サムネイル生成、ジョブ状態更新を行う。
...
Composeでアプリケーションを起動する場合も、PostgreSQL、Elasticsearch、RabbitMQ、保存領域のデータを不用意に削除しない。
...
- PostgreSQL、Elasticsearch、ファイルシステム、7-Zip、RabbitMQなどの外部依存は、必要に応じてモック、統合テスト、手動確認を使い分ける。
...
- PostgreSQL、Elasticsearch、RabbitMQへの接続失敗が区別できる。

## よく使う開発コマンド

| 用途 | コマンド例 |
| --- | --- |
| ミドルウェア起動 | `docker compose up -d postgres elasticsearch rabbitmq` |
...
Windows PowerShellで `npm.ps1` が実行ポリシーによりブロックされる場合は、`npm` の代わりに `npm.cmd` を使用する。
...
アプリケーションのディレクトリ構成、ビルドツール、Docker Composeサービス名、起動プロファイル、テストコマンド、ログ形式、管理コマンドが変更された場合は、このドキュメントを更新する。
