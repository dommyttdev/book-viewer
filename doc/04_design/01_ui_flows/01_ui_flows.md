...
| 利用者 | 入口 | 主な到達先 |
| --- | --- | --- |
| 未ログインユーザ | ログイン画面、会員登録画面 | 会員登録、ログイン、パスワードリセット |
...
flowchart TD
  ...
  Public --> PasswordReset[パスワードリセット]
  Register --> EmailVerify[メール認証]
  EmailVerify --> Login
  Login --> Login2fa[メール2段階認証]
  Login2fa --> BookList[本一覧]
  PasswordReset --> Login
  ...
  AdminLogin --> Admin2fa[管理メール2段階認証]
  Admin2fa --> AdminDashboard[管理ダッシュボード]
...
3. 初回認証に成功したら、メール2段階認証画面へ遷移する。
4. メールで受け取った認証コードを入力する。
5. 認証成功後、一般ユーザは本一覧画面へ遷移する。

### 主な分岐

| 分岐 | 画面の扱い |
| --- | --- |
| 認証失敗 | メールアドレスまたはパスワードが不正であることを安全に表示する。 |
| メール未認証 | メール認証の再送導線を表示する。 |
| 2段階認証コード不正 | 再入力できる状態を維持する。 |
...
3. 次ページ、前ページ、ページ番号指定で移動する。
4. ページ移動時または一定間隔で閲覧位置を保存する。
5. ビューアから本詳細または本一覧へ戻る。
...
| --- | --- |
| 1ページ表示 | MVPの基本表示とする。 |
| 見開き表示 | Beta / v1.0候補として、画面幅が十分な場合に提供する。 |
| 次ページ / 前ページ | ボタン、キーボード、タッチ操作で提供する候補とする。 |
| ページ番号指定 | Beta / v1.0候補として扱うが、画面メモには入力欄候補を残す。 |
| 拡大縮小 | Beta / v1.0候補として扱う。 |
| スマートフォン操作 | タップ領域またはスワイプでページ送りできる設計を候補とする。 |
...
1. 管理ユーザが管理ログイン画面を開く。
2. 管理ユーザ用のメールアドレスとパスワードを入力する。
3. 初回認証に成功したら、管理用メール2段階認証画面へ遷移する。
