...
対象は、index name、alias、mapping、analyzer、ICU normalizer、edge n-gram系フィールド、検索対象、ソート対象、boost、更新タイミング、Outbox状態、再インデックス方針である。
...
検索インデックス更新は、[検索設計の検索更新Outbox](01_search_design.md#検索更新outbox)を正本とする。APIまたは変換ワーカーはPostgreSQL更新時に`search_index_outbox`を作成し、コミット後に即時Elasticsearch更新を試みる。成功時はOutboxを`completed`へ更新し、失敗時は`retry_waiting`としてPostgreSQLに記録したうえでRabbitMQへ再試行通知を送る。

RabbitMQはOutbox IDを配送する通知手段であり、検索更新状態の正本ではない。再試行ワーカーはOutbox IDからPostgreSQLを読み直し、失敗時点の差分ではなく最新正本から検索ドキュメントを再生成する。

初期状態は次のとおりとする。状態の意味と遷移の詳細は検索設計を正本とする。

| 状態 | インデックス設計上の扱い |
| --- | --- |
| `pending` | 正本更新は完了し、Elasticsearch更新待ち。 |
| `processing` | API、変換ワーカー、再試行ワーカーのいずれかが更新中。 |
| `completed` | 最新正本からの更新が完了済み。 |
| `retry_waiting` | 更新失敗後、次回再試行待ち。 |
| `dead_letter` | 自動再試行上限に達し、手動確認または再インデックスが必要。 |
...
6. 処理結果、失敗理由、再試行可否をログとPostgreSQLの`search_index_outbox`へ記録する。

書籍単位再インデックスに成功した場合は、同じ書籍を対象にした古い未完了Outboxを完了または不要扱いへ整理できるようにする。整理方法の物理状態名は実装時に決めるが、検索ドキュメント自体は常に最新正本から再生成する。
...
9. 未完了の`search_index_outbox`を確認し、全件再インデックス後も再試行が必要なものだけを残す。
