...
- 管理ユーザは、`book.view` 権限を持つ場合に管理対象書籍の内容確認・品質確認として閲覧できる。
- 変換済みで、一般ユーザの閲覧または管理ユーザの確認対象として許可されたページのみ画像配信対象にする。
...
対象書籍が非表示、変換未完了、論理削除済みの場合、一般ユーザには閲覧不可として扱う。管理ユーザ向けの内容確認・品質確認では、公開状態にかかわらず確認対象にできるかを[権限マトリクス](../08_authorization_design/02_permission_matrix.md)に従って判定する。
...
| 操作 | 一般ユーザ | 管理ユーザ |
| --- | --- | --- |
| ページ一覧 / 画像 / サムネイル | 閲覧可能書籍のみ可 | `book.view` を持つ場合、管理対象書籍の内容確認・品質確認として可 |
| 閲覧位置保存 | 自分のみ可 | 不可 |
| お気に入り | 自分のみ可 | 不可 |
...
- 画像配信のキャッシュ方針。
- 管理ユーザ向けの内容確認・品質確認を一般向けAPIと共通化するか、管理向けAPIに分けるか。
