...
アップロード1ファイルには技術的な安全上限を設ける。具体値は [画像変換設計のリソース制限と設定](../07_image_conversion_design.md#リソース制限と設定) を正本とする。これは業務上の保存容量上限ではなく、巨大ファイルや同時アップロードによるストレージ枯渇を防ぐための制限である。上限を超える場合は`413 Payload Too Large`相当の安全な業務エラーとして返し、原本保存、書籍登録、変換ジョブ作成は行わない。

同時アップロード数にも安全上限を設ける。上限を超える場合は`429 Too Many Requests`または同等の一時的な受付不可として扱い、時間を置いて再試行できる安全な応答にする。
...
- 公開IDの形式。
- アップロードサイズや同時アップロード数の安全上限を実装時の設定名に合わせて具体化する。
